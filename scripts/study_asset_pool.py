...
def main():
    ...
    paths_latest = [m["path"] for m in manifest.values()] + [
    ...
    source_hashes = {}
    for name, paths, count in [("merged62", paths_latest, 62), ("merged22", paths_earlier, 22)]:
        frames = [pd.read_parquet(p) for p in paths]
        assert all(set(f.day) == set(frames[0].day) for f in frames)
        merged = pd.concat(frames, ignore_index=True).sort_values(["day", "symbol"])
        assert not merged.duplicated(["day", "symbol"]).any()
        assert merged.symbol.nunique() == count
        merged.to_parquet(destination / (name + ".parquet"), index=False)
        for path in paths:
            source_hashes[path] = hashlib.sha256(Path(path).read_bytes()).hexdigest()
        for symbol in merged.symbol.unique():
            old_path = Path("artifacts/research/sec-quality") / f"{symbol}-facts.json"
            new_path = CACHE / f"{symbol}-facts.json"
            src = old_path if old_path.exists() else new_path
            if old_path.exists() and new_path.exists():
                assert old_path.read_bytes() == new_path.read_bytes()
            shutil.copyfile(src, facts / src.name)
    ...
    (ROOT / "2026-09-22-asset-pool-data.json").write_text(
        json.dumps(source_hashes, indent=2) + "\n"
    )
    rows = []
    diagnostics = []
    dest = ROOT / "2026-09-22-asset-pool.json"
    for pool, path, start, end in windows:
        ...
        maps, coverage, _ = forecasts(
            frame, start, end, judge_fn=judge, ratio_key="growth", **kwargs
        )
    ...
    (ROOT / "2026-09-22-asset-pool-coverage.json").write_text(
