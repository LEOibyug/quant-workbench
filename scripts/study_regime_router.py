"""Compare frozen causal regime filter, emission-only and constant routing."""
...
from regime_router import features, probabilities
...
def mappings(frame, bundle):
    ...
    x = features(logs)
    policies = np.eye(4)[bundle["binding"]]
    outputs = {m: {} for m in ("markov", "emission", "constant")}
    posterior = {m: probabilities(bundle, x, m) for m in outputs}
    ...
    for i in range(63, len(close)):
        ...
        for mode in outputs:
            mix = posterior[mode][i - 63] @ policies
...
def main():
    artifact = Path("artifacts/models/regime-router-v0/model.joblib")
    training = json.loads((ROOT / "2026-09-21-regime-router-training.json").read_text())
    ...
    results = []
    destination = ROOT / "2026-09-21-regime-router.json"
            ...
            for mult in (1, 2):
                ...
                results.append(
                    dict(
                        ...
                        mean_policy_probabilities=diagnostics[mode],
                        config=r["config"],
                        metrics=r["metrics"],
