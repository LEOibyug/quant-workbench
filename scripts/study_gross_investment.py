"""Frozen gross-profitability and conservative-investment comparison."""
...
import numpy as np
import pandas as pd
from gross_investment_features import judge
...
def forecasts(frame):
    ...
    out = {m: {} for m in ("gross", "combined", "eligible")}
    diagnostics = []
    example = None
    evaluation_days = [str(d) for d in p.index if "2025-09-01" <= str(d) < "2026-09-01"]
    ...
    for i in range(63, len(p)):
        ...
        eligible = sorted(s for s in syms if judgments[s].get("candidate_eligible"))
        n = math.ceil(len(eligible) / 3)
        gross = sorted(eligible, key=lambda s: (-judgments[s]["gross"], s))
        growth = sorted(eligible, key=lambda s: (judgments[s]["growth"], s))
        combined = sorted(eligible, key=lambda s: (gross.index(s) + growth.index(s), s))
        sets = (
            dict(gross=gross[:n], combined=combined[:n], eligible=eligible)
            if len(eligible) >= 3
            else dict(gross=[], combined=[], eligible=[])
        )
...
def main():
    ...
    results = []
    diagnostics = {}
    output = ROOT / "2026-09-21-gross-investment.json"
    for pool, meta in manifest.items():
        ...
        (ROOT / "2026-09-21-gross-investment-coverage.json").write_text(
