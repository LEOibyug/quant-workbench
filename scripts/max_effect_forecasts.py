"""Lottery-preference MAX candidate: causal observations, not suitability proof."""
import argparse
import json
import numpy as np
import pandas as pd
...
def forecasts(frame,start,end):
    ...
    out={k:{} for k in ['low_max','high_max','eligible']}
    diagnostics=[]
    for i in range(63,len(p)):
        day=str(p.index[i])
        if day not in decisions: continue
        maxes=np.max(simple[i-21:i],axis=0)
        order=sorted(range(len(syms)),key=lambda j:(maxes[j],syms[j]))
        n=len(syms)//3
        selected=dict(low_max=order[:n],high_max=order[-n:],eligible=list(range(len(syms))))
        ...
        diagnostics.append(dict(day=day,max_return=dict(zip(syms,maxes.tolist())),
...
def main():
    ...
    if frame.empty or symbol not in set(frame.symbol):
        ...
        result=dict(symbol=symbol,asof=a.date,observation_close=day,status='证据不足',validated=False,
                    max_return=d['max_return'][symbol],rank=d['rank'][symbol],pool_size=frame.symbol.nunique(),
                    candidate_group=next((k for k in ['low_max','high_max'] if symbol in d['selected'][k]),'middle'),
