"""Dollar-volume price-response diagnostic; frozen 30-account experiment."""
...
from illiquidity_forecasts import forecasts
...
def main():
    ...
    (ROOT / "2026-09-22-illiquidity-data.json").write_text(
        json.dumps(source_hashes, indent=2) + "\n"
    )
    rows = []
    diagnostics = []
    dest = ROOT / "2026-09-22-illiquidity.json"
    for pool, path, start, end in windows:
        ...
        maps, coverage = forecasts(frame, start, end)
        middle = coverage[len(coverage)//2]['day']
        earlier, prior_coverage = forecasts(frame[frame.day<=middle], start, end)
        ...
        for decision in coverage:
            ...
            assert not set(groups["low_illiquidity"]) & set(groups["high_illiquidity"])
            for mapping in maps.values():
                weights = [
                    v["target_weight"] for (day, _), v in mapping.items() if day == decision["day"]
                ]
                assert all(0 <= w <= 0.2 + 1e-10 for w in weights)
                assert sum(weights) <= min(.95,.2*(frame.symbol.nunique()//3)) + 1e-10
            ...
            def record(method, r, pool=pool, start=start, end=end, mult=mult):
                rows.append(
                    dict(
                        ...
                        start=start,
                        end_exclusive=end,
                        method={"low_illiquidity":"low_illiquidity","high_illiquidity":"high_illiquidity","eligible":"eligible"}[method],
    ...
    (ROOT / "2026-09-22-illiquidity-coverage.json").write_text(
