"""Inventory/cost hypothesis; frozen 72-account development diagnostic."""
...
import pandas as pd
from audit_inventory_quality import judge
...
def main():
    ...
    rows = []
    diagnostics = []
    dest = ROOT / "2026-09-22-inventory-strategy.json"
    for pool, path, start, end in windows:
        ...
        maps, coverage, _ = forecasts(
            frame, start, end, judge_fn=judge, ratio_key="inventory_cost_ratio", **kwargs
        )
        for decision in coverage:
            groups = decision["selected"]
            assert not set(groups["nonincrease"]) & set(groups["increase"])
    ...
    (ROOT / "2026-09-22-inventory-strategy-coverage.json").write_text(
