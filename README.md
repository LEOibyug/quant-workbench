...
长期任务完成后可“发布到展示页”，历史记录可恢复并再次查看或发布。发布保存独立、不可变的股票范围、统计估计方法、成本与风控参数，不复制研究行情、收益曲线或成交记录。展示页选择该版本和覆盖全部股票的行情数据集后，可运行隔夜组合模拟，查看日终净值、持仓和分批成交，导出CSV；已发布参数不能在模拟请求中覆盖。统计模型在模拟过程中仅依据当时可用历史重新估计，不发布单个研究结束时刻的拟合权重。发布表示可展示的研究版本，不代表通过实盘准入。
